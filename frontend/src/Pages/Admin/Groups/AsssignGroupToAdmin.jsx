...
const AssignGroupToAdmin = () => {
  ...
  const GroupAssignAPI = async () => {
    setIsAssigning(true);
    try {
      const url = `https://grandurenet-main.onrender.com/api/user/assigngroup/${selectedAdmin._id}`;
      const headers = {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: `Bearer ${accessToken}`
    ...
    <>
      ...
      >
        <Box sx={modalStyle}>
          {selectedAdmin && (
            <>
              <Typography id="assign-group-modal" variant="h6" component="h2" gutterBottom>
                ...
                {groups.map((group) => (
                  ...
                  >
                    <ListItemText
                      ...
                      secondary={`Type: ${group.grouptype} • Members: ${group.users?.length || 0}`}
                    />
                  </ListItem>
                ))}
              </List>
              ...
              <Box mt={3} display="flex" justifyContent="flex-end">
                ...
                <Button
                  ...
                  startIcon={isAssigning ? <CircularProgress size={20} /> : <CheckCircle />}
                >
